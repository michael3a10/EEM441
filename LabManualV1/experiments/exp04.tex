...
% ============================================================
\experimentheader{4}{DIP Switch \& 7-Segment}{Interfacing FPGA to DIP Switch and Seven Segment}
...
\begin{enumerate}
  \item Open Quartus II and write the frequency-divider module in Listing~\ref{lst:exp4-seven} (\texttt{seven}), which lowers the incoming clock frequency.
...
\begin{codepanel}{seven.v --- frequency divider}
...
\end{codepanel}
\vspace{-4pt}
\begin{center}\small\color{slategray}Listing \refstepcounter{equation}\theequation\label{lst:exp4-seven}: \texttt{seven.v} --- 16-bit frequency divider.\end{center}

\begin{codepanel}{led.v --- 4-bit counter}
\begin{minted}{verilog}
module led(x, clk);
  input clk;
  output [3:0] x;
...
\end{codepanel}
\vspace{-4pt}
\begin{center}\small\color{slategray}Listing \refstepcounter{equation}\theequation\label{lst:exp4-led}: \texttt{led.v} --- 4-bit binary counter.\end{center}

\begin{boxedfigure}
\centering
\begin{tikzpicture}[node distance=10mm and 14mm]
  \node[io] (clk) {clk};
  \node[block, right=of clk, minimum width=2.4cm, minimum height=1.8cm] (seven) {seven\\[2pt]\scriptsize clk~~q[15..0]};
  \node[block, right=32mm of seven, minimum width=2.2cm, minimum height=1.8cm] (led) {led\\[2pt]\scriptsize clk~~x[3..0]};
  \node[io, right=of led] (out) {x[3..0]};

  \draw[arr] (clk) -- (seven);
  \draw[arr] (seven.east) -- (led.west) node[midway, above, font=\scriptsize] {q[15..0]};
  \draw[arr] (led) -- (out);

  % tap q[16] (MSB of divider) down and back up into led clock
  \coordinate (tap) at ($(seven.east)!0.35!(led.west)$);
  \draw[arr] (tap) -- ++(0,-1.1) node[below, font=\scriptsize] {q[16]} -| (led.south);
\end{tikzpicture}
...
\midrule
1 & clk  & Input  & PIN\_D25 \\
2 & X[3] & Output & PIN\_AC22 \\
3 & X[2] & Output & PIN\_AB21 \\
4 & X[1] & Output & PIN\_AF23 \\
5 & X[0] & Output & PIN\_AE23 \\
\bottomrule
\end{tabular}
