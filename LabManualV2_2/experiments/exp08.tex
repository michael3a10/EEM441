...
% ============================================================
\experimentheader{8}{Experiment 8 (RPi, open-ended)}{Temperature Control Using Raspberry Pi}{Instrumentation Lab}
...
\begin{boxedfigure}
  ...
  \begin{tikzpicture}[node distance=8mm and 10mm]
    ...
    \node[block, right=of ctrl, minimum width=2.4cm] (heater) {Heater};
    \node[block, left=of ctrl, minimum width=2.2cm] (buf) {Buffer};
...
\begin{procedure}
  ...
  \begin{boxedfigure}
    ...
    \begin{tikzpicture}[node distance=8mm and 12mm]
      \node[io] (gpio) {Raspberry Pi\\GPIO};
      \node[block, right=of gpio] (bjt) {BJT};
      \node[block, right=of bjt, minimum width=2.6cm] (relay) {Relay\\\scriptsize pins 1/2/3, 6/5/4};
      \node[io, above=of relay, yshift=-4mm] (v5) {5\,V};
      \node[io, right=of relay, yshift=6mm] (v12) {12\,V};
      \node[block, right=of relay, yshift=-8mm, minimum width=2.6cm] (heaterterm) {To heater\\(trainee terminal)};

      \draw[arr] (gpio) -- (bjt);
      \draw[arr] (bjt) -- (relay);
      \draw[arr] (v5) -- (relay.north);
      \draw[arr] (v12) -- ($(relay.east)+(0,4mm)$);
      \draw[arr] ($(relay.east)+(0,-4mm)$) -- (heaterterm);
    \end{tikzpicture}
